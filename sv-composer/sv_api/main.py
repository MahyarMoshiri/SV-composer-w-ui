...
def _cors_enabled() -> bool:
    toggle = os.getenv("SV_API_ENABLE_CORS")
    if toggle is None:
        # Default to enabling CORS to support local web clients (disable via env).
        return True
    return toggle.strip().lower() in {"1", "true", "yes", "on"}


def _cors_origins() -> list[str]:
    raw = os.getenv("SV_API_CORS_ORIGINS", "*")
